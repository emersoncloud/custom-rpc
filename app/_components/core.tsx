                    ...
                    <div>
                        <>
                            ...
                            <div>
                                {showAddRpc ? (
                                    <div>
                                        <input type="text" placeholder={"Add a custom RPC URL"} value={newRpc} onChange={(e) => setNewRpc(e.target.value)} className="flex-grow" />
                                        <div>
                                            <button name="add-rpc" onClick={handleAddRpc} disabled={!newRpc}>
                                                (add rpc)
                                            </button>
                                            <button onClick={() => setShowAddRpc(false)}>(remove)</button>
                                        </div>
                                    </div>
                                ) : (
                                    <button className="w-full" onClick={() => setShowAddRpc(true)}>
                                        Add Custom RPC
                                    </button>
                                )}

                                {transports?.[chainId]?.slice(0, -1).map((rpcUrl: string, _index: number) => (
                                    <div key={rpcUrl}>
                                        <div>
                                            <input type="text" value={rpcUrl} />
                                            <button name="remove-rpc-listed" onClick={() => handleRemoveRpc(rpcUrl)}>
                                                (remove)
                                            </button>
                                        </div>
                                    </div>
